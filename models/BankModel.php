...
class BankModel extends BaseModel
{
    ...
    public function updateBank($input)
    {
        ...
        $cost = $input['cost'];
        if (
            is_object($id) || is_string($id) || $id < 0 || is_double($id) || empty($id) || is_bool($id)
            || is_object($cost) || is_string($cost) || $cost < 0 || is_double($cost) || empty($cost) || is_bool($cost)
        ) {
            return false;
        } else {
            $sql = 'UPDATE banks SET cost = ' . $cost . ' WHERE id =' . $id;
            // var_dump($sql);
            // die();
