...
class CommentController extends Controller
{
    ...
    public function postUpdateComment($id, Request $request)
    {
        $comment = DB::table('user_comment')
            ...
            ->where('comment_id', $id)->get();
        $data = array();
        if ($request->version == $comment[0]->comment_version) {
            $data['user_id'] = $request->user_id;
            $data['hotel_id'] = $request->hotel_id;
            $data['rating'] = $request->rating;
            $data['content'] = htmlspecialchars($request->content, ENT_QUOTES, 'UTF-8');
            $data['time_cmt'] = date('Y-m-d');
            $data['comment_version'] =  $comment[0]->comment_version + 1;
    ...
    {
        $id = substr($id, 9);
        DB::table('user_comment')->where('comment_id', $id)->delete();
        return Redirect::to('/admin/comment');
    }
}
